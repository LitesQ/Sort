...
try:
    ...
    for i in range(len(result)):
        ...
        doc_full_raw1 = str(result[i+1][8]).replace(" ","")

        try:
            if doc_full_raw1.find("сер.") != -1:
                serial1 = doc_full_raw1.split("сер.")[1].split("№")[0]
                numb1 = doc_full_raw1.split("сер.")[1].split("№")[1]
            else:
                serial1 = doc_full_raw1[0:4]
                numb1 = doc_full_raw1[4:]
        except:
            numb1 = doc_full_raw1
            serial1 = ""
        # Подключение к существующей базе данных
        ...
        # Курсор для выполнения операций с базой данных
        cursor = connection.cursor()
        query = "SELECT * FROM civilians WHERE doc_full = {}".format("'"+serial1+numb1+"'")
        cursor.execute(query)
        ...
        query = "UPDATE civilians SET last_name = {},first_name = {},patronymic = {},fio = {},date_of_birth = {},birth_region = {},street = {},dom = {},korpus = {},stroenie = {},kvartira = {},tel = {},sex = {},citizenship = {},doc_type = {},doc_serial = {},doc_number = {},doc_full = {},doc_org = {},doc_date = {},registration_type = {},registration_start = {},registration_end = {},to_del = {} WHERE doc_full = {}".format("'"+last_name+"'","'"+first_name+"'","'"+patronymic+"'","'"+last_name + " " + first_name + " " + patronymic+"'","'"+birth_date+"'","'"+birth_region+"'","'"+street+"'","'"+home+"'","'"+korpus+"'","'"+""+"'","'"+kvartira+"'","'"+""+"'","'"+sex+"'","'"+citizenship+"'","'"+doc_type+"'","'"+serial+"'","'"+numb+"'","'"+serial+numb+"'","'"+doc_org+"'","'"+doc_date+"'","'"+"Постоянная"+"'","'"+""+"'","'"+""+"'","'"+""+"'","'"+serial1+numb1+"'")
        cursor.execute(query)
        connection.commit()
        print(str(i))
        count = cursor.rowcount
